...
screen::screen()
{
    ...
    l_tiempo = new QLabel("Tiempo de ejecucion:",container);
    l_tiempo->move(450,150);
    l_tiempo->setVisible(false);
...
void screen::onFinishedOp()
{
    pb_operar->setVisible(false);
    op_vis(true);
    finish = QTime::currentTime();
    l_tiempo->setText(QString("Tiempo: ").append(QString::number(start.msecsTo(finish)/1000.0)));
}
